...
func HandleFeed(w http.ResponseWriter, r *http.Request, p httprouter.Params) {
	...
	// Retrieve the videos.
	url := fmt.Sprintf("http://%s/video/feed/%s/%s", BACKEND_URL, amount, page)
...
func GetVideoFromRank(rank int) (*video.VideoWithUserEntry, error) {
	...
	// Perform the request.
	url := fmt.Sprintf("http://%s/video/rank/%d", BACKEND_URL, rank-1)
	resp, err := http.Get(url)
...
func GetMyVideos(w http.ResponseWriter, r *http.Request, _ httprouter.Params) {
	...
	url := fmt.Sprintf("http://%s/users/%s/videos", BACKEND_URL, username)
	resp, err := http.Get(url)
	if err != nil {
		log.Println(err)
		w.WriteHeader(http.StatusInternalServerError)
	...
	for _, v := range response.Videos {
		url := fmt.Sprintf("http://%s/users/%s/videos/%s/info", BACKEND_URL, username, v.Key)
...
func HandleEditPage(w http.ResponseWriter, r *http.Request, p httprouter.Params) {
	...
	// Request for the video information.
	url := fmt.Sprintf("http://%s/users/%s/videos/%s/info", BACKEND_URL, username, videoKey)
	resp, err := http.Get(url)
	if err != nil {
		log.Println("Could not access!", err)
		return
